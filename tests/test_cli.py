...
def test_exibe_aquisicao_descontada():
    res = runner.invoke(cli, ["--aquisicao-descontada", "0.61", "27.18", "26.06"])
    assert res.output == "25.45\n"
    res = runner.invoke(cli, ["-ad", "0.61", "27.18", "26.06"])
    assert res.output == "25.45\n"
    assert res.exit_code == 0
